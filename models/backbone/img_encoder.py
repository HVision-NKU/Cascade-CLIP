...
class VPTCLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.0, out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings=False, 
        ...
        self.out_indices = out_indices
        print("--------------------------------------------------------------------\n")
        print(f'Extract {out_indices} and last layer features from the image encoder.') 

        if get_embeddings:
            self.ln_post = LayerNorm(width)
            self.proj = nn.Parameter(scale * torch.randn(width, output_dim))

            self.ln_layers = nn.ModuleList([LayerNorm(width) for _ in range(len(out_indices))])
            # self.proj_layers = nn.ParameterList([nn.Parameter(scale * torch.randn(width, output_dim)) for _ in range(len(out_indices))])
            self.proj_layers =  nn.Parameter(scale * torch.randn(len(out_indices), width, output_dim))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.get_embeddings:
            ...
            for idx, (feature, ln_post, proj) in enumerate(zip(features, self.ln_layers, self.proj_layers)):
                feature = feature.permute(1, 0, 2)
                feature = ln_post(feature)   
                feature = feature @ proj
                feature = feature[:, -(H*W):].reshape(B, H, W, -1).permute(0, 3, 1, 2)
